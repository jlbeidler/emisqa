...
def get_spec(species_name, opts):
    '''
    Sums the hourly values of NCF files from a start date through the number of run dates.
    '''
    if opts.verbosity: 
    ...
    for day in range(opts.run_days):
        ...
        if opts.sector.lower() == 'mrggrid':
            infile_name = os.path.join(opts.inpath, 'emis_mole_all_%s_%s_nobeis_%s.ncf' %(current_day, opts.grid, opts.case))
        else:
            infile_name = os.path.join(opts.inpath, opts.sector, 
              '%s_mole_%s_%s_%s_%s_%s.ncf' %(inprefix, opts.sector, current_day, opts.grid, 
              opts.spec, opts.case))  # v5 directory structure 
        infile = DataFile(infile_name, verbosity, inFormat, ptsr, opts.zip_dict)
        if species_name not in infile.species_list and opts.ignore_spec:
            ...
            break
        in_array = infile.dump_val(species_name, opts.all_hours, opts.grid, opts.ignore_spec, 
          opts.inln, opts.interpolate, opts.layer, opts.stacks) * day_mult
        if day == 0: 
